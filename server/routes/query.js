...
router.post("/add", async (req, res) => {
    ...
    try {
        // Find an existing query for the user, or create a new one if not found
        let userQuery = await Query.findOne({ userId });

        if (!userQuery) {
            // Create a new query if none exists
            userQuery = new Query({
                userId,
                messages: [{ sender, message }],
            });
        } else {
            // If the user already has an ongoing query, push the new message into the array
